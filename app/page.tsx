...
export default function LoginPage() {
  ...
  return (
    <div
      className="relative flex min-h-screen flex-col bg-[#faf9f6] dark:bg-[#09090f]"
      style={{
        backgroundImage: 'radial-gradient(circle, var(--dot-color) 1px, transparent 1px)',
        backgroundSize: '24px 24px',
      }}
    >
      {/* Amber brand stripe — top */}
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-16">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8 text-center">
            <div
              className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[#f59e0b] text-xl font-extrabold text-white"
              style={{ fontFamily: 'var(--font-syne)' }}
            >
              C
            </div>
            <h1
              className="text-4xl font-extrabold text-slate-900 dark:text-[#f0f0f6]"
          ...
          {/* Login card */}
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-[#22222e] dark:bg-[#111119]">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full bg-[#f59e0b] text-[#09090f] hover:bg-[#fbbf24] dark:bg-[#f59e0b] dark:text-[#09090f] dark:hover:bg-[#fbbf24]"
                disabled={loading}
              >
              ...
              <div className="grid grid-cols-3 gap-2">
                {DEMO_ROLES.map(({ role, label, color, bg }) => (
                  <button
                    ...
                    className="rounded-lg border py-2 text-xs font-semibold transition-colors hover:opacity-80 disabled:opacity-40"
                    style={{ borderColor: color + '40', background: bg, color }}
                  >
                    {label}
                  </button>
                ))}
